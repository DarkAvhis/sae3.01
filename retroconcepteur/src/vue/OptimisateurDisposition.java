 ...
 * en utilisant un algorithme hiérarchique (Sugiyama-style) qui minimise les
...
public class OptimisateurDisposition {
     ...
     * Les blocs sont arrangés en couches horizontales pour minimiser les
     * croisements de liaisons.
     * 
    ...
    /**
     * Calcule la couche (layer) de chaque bloc basée sur sa profondeur dans la
     * hiérarchie.
     * Les classes filles (enfants) sont à la couche 0 (en haut), leurs parents plus
     * bas.
     */
    private static HashMap<String, Integer> calculerCouches(List<BlocClasse> blocsClasses,
        ...
        for (BlocClasse bloc : blocsClasses) {
            if (couches.get(bloc.getNom()) == -1) {
                calculerProfondeur(bloc.getNom(), couches, dependances, new HashSet<String>());
            }
        }
    ...
    private static int calculerProfondeur(String nomBloc, HashMap<String, Integer> couches,
        ...
        Integer coucheExistante = couches.get(nomBloc);
        if (coucheExistante != null && coucheExistante != -1) {
            return coucheExistante;
        }

        if (enCours.contains(nomBloc)) {
            // Cycle détecté : assigner une profondeur minimale pour casser la récursion
     ...
     */
    private static HashMap<Integer, List<BlocClasse>> groupeParCouche(List<BlocClasse> blocsClasses,
    ...
    /**
     * Positionne les blocs dans le panneau basé sur leurs couches avec optimisation
     * spatiale.
     * Les blocs sont arrangés pour minimiser les croisements et créer une
...

    {
        ...
        for (int couche = 0; couche < maxCouche; couche++) {
            ...
            // Plus de place pour les blocs : adapter l'espacement en fonction du nombre de
            // blocs
...

    {
        ...
        for (BlocClasse bloc : blocsEnCouche) {
            ...
            // Chercher les blocs de la couche précédente/suivante pour calculer une
            // position optimale
            for (BlocClasse autreBlocCouche : blocsEnCouche) {
                if (bloc.getNom().equals(autreBlocCouche.getNom()))
                    continue;
