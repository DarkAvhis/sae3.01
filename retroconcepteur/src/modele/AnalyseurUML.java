...
public class AnalyseurUML
{
    ...
    public List<AssociationObjet> detecterAssociations(List<ClasseObjet> classes, HashMap<String, ClasseObjet> mapClasses)
    {
        ...
        {
            if (associationsDansFusion.contains(assoOrigine)) continue;
            ...
            if (pairesDeClassesFusionnees.contains(clePaire)) continue;
            
            AssociationObjet assoInverse = null;
            for (AssociationObjet a : associationsUnidirectionnelles)
            {
                if (!associationsDansFusion.contains(a) && a.getClasseFille() == classeB && a.getClasseMere() == classeA)
                {
                    assoInverse = a;
                    break;
                }
    ...
    /**
     * Résout les héritages à partir des intentions stockées dans lstIntentionHeritage,
     ...
     * en utilisant mapClasses pour retrouver les ClasseObjet. Retourne la liste d'InterfaceObjet.
     */
    public List<InterfaceObjet> resoudreImplementation(HashMap<String, ClasseObjet> mapClasses)
    {
        ...
        for (String nomClasseConcrète : this.lstInterfaces.keySet())
        {
            ...
            {
                System.out.println("Avertissement: Classe concrète '" + nomClasseConcrète + "' déclarée pour implémentation mais non trouvée.");
                continue;
            }
            ...
            {
                if (nomInterfaceStr == null || nomInterfaceStr.trim().isEmpty()) continue;

                if (!mapClasses.containsKey(nomInterfaceStr))
                {
                    System.out.println("Avertissement: Interface '" + nomInterfaceStr + "' implémentée par '" + nomClasseConcrète + "' non trouvée dans le répertoire.");
                    continue;
                }
